...
GSF.chat = (function () {
    ...
    function init(gid, base, uid, leader) {
        ...
        /* ── study session button (leader only) ── */
    ...
    function renderMessages(msgs, initial) {
        ...
        for (var i = 0; i < msgs.length; i++) {
            ...
            /* Study session cards are never grouped */
            ...
            if (m.message_type === 'study_session') {
                /* Full-width session card – no avatar grouping */
    ...
    /* Init all audio players not yet initialised */
    ...
    function buildStudySessionCard(session) {
        ...
        return '<div class="ssc" data-session-id="' + session.id + '">' +
            ...
            '<div class="ssc_foot">' +
                ...
                '</span>' +
                '<button type="button" class="ssc_join_btn' + (joined ? ' joined' : '') + '" ' +
                    'onclick="GSF.chat.toggleSession(' + session.id + ', this)">' +
                    (joined ? '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg> Joined' : 'Join Session') +
                '</button>' +
            '</div>' +
        '</div>';
    }
    ...
    function sendStudySession() {
        ...
        fetch(baseUrl + '/php/api/study_sessions.php', {
            method:  'POST',
            headers: { 'Content-Type': 'application/json' },
            body:    JSON.stringify({ action: 'create', group_id: groupId, title, date, time, location, link })
        }).then(function (r) { return r.json(); })
          .then(function (data) {
    ...
    /* Called from inline onclick in session card */
